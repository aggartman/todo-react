...
- [ ] Extract localStorage read/write into a small `useLocalStorage` custom hook — currently duplicated logic in two `useEffect`s in `App.js`. (Do this _after_ the lazy-initializer fix above; the hook should encapsulate both the seed-from-storage and the persist-on-change halves.)
